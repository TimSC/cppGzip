...
/*
I hacked the original libtar so it supports in memory reading and writing 
(with streaming!), as well as ways for C++ to hook into it. TSC 2019
*/
...
typedef void * (*openfunc_t)(void *, const char *, int, mode_t mode, ...);
typedef int (*closefunc_t)(void *, void *);
...
typedef struct
{
	...
	openfunc_t outopenfunc;
	closefunc_t outclosefunc;
	readfunc_t outreadfunc;
	writefunc_t outwritefunc;
}
tartype_t;
...
int tar_fdopen(TAR **t, void *fd, const char *pathname, tartype_t *type,
	       int oflags, int mode, int options, void *opaque);
...
#define tar_block_read(t, buf) \
	(*((t)->type->readfunc))(t->opaque, (t)->fd, (char *)(buf), T_BLOCKSIZE)
#define tar_block_write(t, buf) \
	(*((t)->type->writefunc))(t->opaque, (t)->fd, (char *)(buf), T_BLOCKSIZE)
